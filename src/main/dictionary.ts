...
const UA = 'Xook/1.0 (desktop reading app; contact: local user)'
const TIMEOUT = 5000
...
async function esWiktionary(word: string, depth = 0): Promise<DictEntry | null> {
  const data = await getJson<EsParse>(
    `https://es.wiktionary.org/w/api.php?action=parse&page=${encodeURIComponent(word)}&prop=wikitext&format=json&formatversion=2&redirects=1`
  )
