...
// ---- A–Z index ----
// Position (in the sorted list) of the first book whose title/author starts at or after the letter
export function indexForLetter(
  by: 'title' | 'author',
  letter: string,
  universeId: string | null
): number {
  const col = by === 'author' ? "COALESCE(authorNorm,'')" : "COALESCE(titleNorm,'')"
  const l = normalize(letter).slice(0, 1)
  if (!/^[a-z]$/.test(l)) return 0
  const args: SQLInputValue[] = [l]
  let where = `WHERE ${col} < ?`
  if (universeId) {
    where += ' AND universeId = ?'
    args.push(universeId)
  }
  const row = getDb()
    .prepare(`SELECT COUNT(*) AS c FROM books ${where}`)
    .get(...args) as { c: number }
  return row.c
}

// Letters that have at least one book (so the index can dim the empty ones)
export function lettersAvailable(by: 'title' | 'author', universeId: string | null): string[] {
  const col = by === 'author' ? 'authorNorm' : 'titleNorm'
  const args: SQLInputValue[] = []
  let where = ''
  if (universeId) {
    where = 'WHERE universeId = ?'
    args.push(universeId)
  }
  const rows = getDb()
    .prepare(`SELECT DISTINCT substr(${col}, 1, 1) AS l FROM books ${where}`)
    .all(...args) as { l: string | null }[]
  const out = new Set<string>()
  for (const r of rows) {
    const c = (r.l ?? '').toLowerCase()
    out.add(/^[a-z]$/.test(c) ? c.toUpperCase() : '#')
  }
